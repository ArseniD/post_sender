...
JOB_LINK_FARFETCH = "http://esbaraciap02d.burberry.corp:8500/jms/write-prod?queue=BUR.HYBRIS.QL.FULFILMENT.ORDERSTATUS"
...
def create_parser():
    parser = ArgumentParser(
        ...
        Example send:  post_sender -s one_cart/one_sign/all_cart/all_sign/all_farfetch
        """,
        description="""
        Download and resend yMKT Abandoned Basket/ATG SignUp/Farfetch messages
        """,
        epilog="""
    ...
    optional.add_argument('-s', choices=['one_signup',
                                         ...
                                         'all_signup',
                                         'all_cart',
                                         'all_farfetch'], dest='send', help="Send post request from each file in folder")
...
def main():
    ...
    try:
        ...
        curr_dir = '.'
        subfolder_names = ['abandoned_card', 'signup', 'farfetch']
        ...
        farfetch_folder = download_folder + subfolder_names[2] + '/'
        ...
        if args.email and args.folder and args.date and not args.password:
            ...
            connection = FetchEmail(username=args.email, password=args.password,
                                                         mail_folder=args.folder)

            for num,subject in enumerate(subjects):
                message_list = connection.fetch_messages(subject=subject,
        ...
        if args.send == 'all_farfetch':
            print("\nSending all FARFETCH requests:\n-----------------\n")
            for file_name, content in content_collector(farfetch_folder).iteritems():
                post_data(file_name=file_name, content=content, job_link=JOB_LINK_FARFETCH)
            sys.exit(1)

        else:
            print('Please see "post_sender -h" for more details')
